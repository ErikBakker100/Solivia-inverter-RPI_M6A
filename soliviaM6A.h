...
void update() {
    while(available() > 0) {
		...
		if (bytes.size() == 158) {
			...
			TwoByte ac_p_data;
			ac_p_data.Byte[0] = bytes[0x68];
			ac_p_data.Byte[1] = bytes[0x69];
			FourByte ac_energy_today_data;
			ac_energy_today_data.Byte[0] = bytes[0x74];
			ac_energy_today_data.Byte[1] = bytes[0x75];
			ac_energy_today_data.Byte[2] = bytes[0x76];
			ac_energy_today_data.Byte[3] = bytes[0x77];			
			char etx;
