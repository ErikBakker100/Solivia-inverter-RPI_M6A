...
public:
	...
	void update() override {
		if (timer+5000 > millis()) {
			mqtt_client->publish("waiting for data\r\n");
			timer = millis();
		}
		while(available() > 0) {
			...
			if (bytes.size() == 160) {
				...
				mqtt_client->publish("ac_v1", "%i", ac_v1_data.UInt16);
				mqtt_client->publish("ac_a1", "%i", ac_a1_data.UInt16);
				mqtt_client->publish("ac_p1", "%i", ac_p1_data.UInt16);
				mqtt_client->publish("ac_f1", "%i", ac_f1_data.UInt16);
				mqtt_client->publish("ac_v2", "%i", ac_v2_data.UInt16);
				mqtt_client->publish("ac_a2", "%i", ac_a2_data.UInt16);
				mqtt_client->publish("ac_p2", "%i", ac_p2_data.UInt16);
				mqtt_client->publish("ac_f2", "%i", ac_f2_data.UInt16);
				mqtt_client->publish("dc_v1", "%i", dc_v1_data.UInt16);
				mqtt_client->publish("dc_a1", "%i", dc_v1_data.UInt16);
				mqtt_client->publish("dc_p1", "%i", dc_v1_data.UInt16);
				mqtt_client->publish("dc_v2", "%i", dc_v2_data.UInt16);
				mqtt_client->publish("dc_a2", "%i", dc_a2_data.UInt16);
				mqtt_client->publish("dc_p2", "%i", dc_p2_data.UInt16);
			}
			bytes.clear();
		}
	}
